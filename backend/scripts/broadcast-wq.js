...
import { sendWorldQuantEmail } from '../src/services/emailService.js';
...
async function broadcast() {
  ...
  console.log(`📡 Starting WorldQuant broadcast to ${lines.length} IEEE members...`);
  ...
  for (const line of lines) {
    ...
    const result = await sendWorldQuantEmail(email, firstName, cc);
    if (result.success) {
      sentCount++;
    }
