...
import {
  ...
  applyCategoryFieldDefaults,
  categoryHidesCompensation,
  categoryHidesFunding,
  categoryHidesUpfrontCost,
...
export function PostWithUs({
      ...
      const prefillIdentity = async () => {
        try {
          ...
          const res = await fetch(
            `${API_BASE}/public/me/posts?filterMode=normal`,
            {
          ...
          if (res.ok) {
            ...
            const latest = allPosts.find(
              (p: any) => p.reporter && p.reporter.name,
            );
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        setError(
          'Extraction timed out. The server might be busy. Please try "Enter Manually".',
        );
      } else {
  ...
  const handleBasicInfoEdit = (key: string, newValue: string) => {
    ...
    if (key === "category") {
      const newBasicInfo = applyCategoryFieldDefaults(
        newValue,
      ...
      setParsedData({
        ...
        ...(isProjectCategory(newValue)
          ? {}
          : { isEscrow: false, escrowAmount: undefined }),
        ...(isChallengeCategory(newValue)
          ? { isEscrow: false, escrowAmount: undefined }
          : {}),
      });
      if (!isProjectCategory(newValue)) {
  ...
  const executePublish = async () => {
    ...
    try {
      ...
      let institutionalEndorsement = undefined;

      if (isProjectCategory(category)) {
        if (!endorsementInstitution.trim() || !endorsementContactTitle.trim()) {
          throw new Error(
        ...
        if (kycDocument) {
          ...
          if (!kycRes.ok) {
            ...
            throw new Error(
              kErr.error || "Failed to upload endorsement document.",
            );
          }
        ...
        if (seeksFunding) {
          if (!parsedData.escrowAmount || parsedData.escrowAmount < 100) {
            throw new Error(
              "Funding goal must be at least KES 100 when raising funds.",
            );
          }
          if (!parsedData.basicInfo.providerMpesa?.trim()) {
            throw new Error(
              "M-PESA number is required when raising funds for a project.",
            );
          }
        }
      }
      ...
      const deadline =
        parsedData.extractedFeatures.find((f) => f.feature === "Deadline")
          ?.value || "";
      const applicationLink =
        parsedData.extractedFeatures.find(
          (f) => f.feature === "Application Link",
        )?.value || "";
      const location =
        parsedData.extractedFeatures.find((f) => f.feature === "Location")
          ?.value || "";

      // Validation: Application Link is MANDATORY for most categories UNLESS custom form is enabled
      const isDemoLink =
        applicationLink.toLowerCase().includes("demo") ||
        applicationLink.includes("example.com");

      if (
        !customForm.isEnabled &&
        !isChallengeCategory(category) &&
        category !== "Open Challenge" &&
        (!applicationLink || applicationLink.trim().length < 8 || isDemoLink)
      ) {
        throw new Error(
          `For ${category}, a valid application link is required unless you enable the Custom Form. Please edit the "Application Link" field below.`,
        );
      }

      const projectSeeksFunding = isProjectCategory(category) && seeksFunding;

      const slug = parsedData.basicInfo.title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/(^-|-$)/g, "");
      ...
      const finalOpportunity = {
        ...
        fullDescription:
          parsedData.basicInfo.fullDescription ||
          parsedData.basicInfo.description,
        ...
        isEscrow: projectSeeksFunding ? true : parsedData.isEscrow || false,
        escrowAmount: projectSeeksFunding
          ? parsedData.escrowAmount
          : parsedData.isEscrow
            ? parsedData.escrowAmount
            : undefined,
      ...
      const publishResponse = await fetch(
        `${API_BASE}/public/submit-opportunity`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handlePublishClick = async () => {
    ...
    if (
      verifiedEmail &&
      verifiedEmail.toLowerCase() === reporter.email.toLowerCase()
    ) {
    ...
    <div className="min-h-screen bg-gray-50 pb-16">
        ...
        <div className="bg-[#131ADF] text-white shadow-md rounded-b-3xl">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
            <p className="text-white mb-6 px-4 py-1 uppercase tracking-widest text-sm font-semibold opacity-90 block">
            ...
            <p className="text-blue-50 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
              {viewMode === "post"
                ? editingPostId
                  ? "Update your live opportunity instantly."
                  : "Reach thousands of top-tier African students and young professionals instantly."
            ...
            <>
                    ...
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          value={reporter.role}
                          onChange={(e) =>
                            setReporter({ ...reporter, role: e.target.value })
                          }
                          className="w-full px-5 py-3 rounded-xl border border-gray-200 outline-none focus:border-blue-500 bg-gray-50/50 transition-colors h-auto"
                        />
                      </div>
                        ...
                        <Card className="mt-4 border-blue-100 bg-blue-50/30">
                          <CardHeader className="pb-2">
                            <CardTitle className="text-sm">
                              Request Official Organization Status
                            </CardTitle>
                          </CardHeader>
                          <CardContent className="space-y-3">
                            {requestStatus === "success" ? (
                              <div className="text-green-600 font-medium py-4 text-center">
                                Request sent! We will verify and contact you
                                shortly.
                              </div>
                            ) : (
                              <form
              ...
              {!editingPostId && (
                <div className="mt-16 mb-24">
                  ...
                  {error && !parsedData && (
                    <div className="mt-6">
                      <p className="text-base text-red-500 font-medium p-4 bg-red-50 border border-red-200 rounded-xl">
                        {error}
                      </p>
                    </div>
                  )}
              ...
              {parsedData && (
                ...
                >
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 gap-6 rounded-2xl bg-gray-50/50 p-6 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            handleBasicInfoEdit("title", e.target.value)
                          }
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            handleBasicInfoEdit("provider", e.target.value)
                          }
                          className="w-full px-5 py-3 rounded-xl border border-gray-200 outline-none focus:border-blue-500 bg-white transition-colors h-auto"
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <select
                          ...
                          onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                            handleBasicInfoEdit("category", e.target.value)
                          }
                        >
                          <option value="" disabled>
                            Select a category
                          </option>
                          {POST_WITH_US_CATEGORY_GROUPS.map((group) => (
                      ...
                      {!categoryHidesFunding(parsedData.basicInfo.category) && (
                        <div className="space-y-1.5">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              handleBasicInfoEdit("fundingType", e.target.value)
                            }
                          >
                            ...
                            <option value="Grant / Venture Backed">
                              Grant / Venture Backed
                            </option>
                      ...
                      {!categoryHidesCompensation(
                        ...
                        <div className="space-y-1.5">
                          ...
                          <select
                            className="w-full px-5 py-3 text-sm bg-white border rounded-xl border-gray-200 focus:outline-none focus:border-blue-500 transition-colors h-auto"
                            value={
                              parsedData.basicInfo.compensationType || "N/A"
                            }
                            onChange={(
                      ...
                      {!categoryHidesUpfrontCost(
                        ...
                        <div className="space-y-1.5">
                          ...
                          <select
                            className="w-full px-5 py-3 text-sm bg-white border rounded-xl border-gray-200 focus:outline-none focus:border-blue-500 transition-colors h-auto"
                            value={
                              parsedData.basicInfo.upfrontCost ||
                              "No Upfront Cost"
                            }
                            onChange={(
                              e: React.ChangeEvent<HTMLSelectElement>,
                            ) =>
                              handleBasicInfoEdit("upfrontCost", e.target.value)
                            }
                          >
                            <option value="No Upfront Cost">
                              No Upfront Cost
                            </option>
                            <option value="Has Upfront Cost">
                              Has Upfront Cost
                            </option>
                          </select>
                        </div>
                      ...
                      {categoryShowsProjectFunding(
                        ...
                        <div className="space-y-4 md:col-span-2 border border-blue-100 bg-blue-50/50 rounded-xl p-5">
                          <label className="flex items-start gap-3 cursor-pointer">
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setSeeksFunding(checked);
                                setParsedData({
                                  ...parsedData,
                                  isEscrow: checked,
                                  escrowAmount: checked
                                    ? parsedData.escrowAmount || 50000
                                    : undefined,
                                });
                              }}
                            />
                            <div>
                              <span className="text-sm font-bold text-gray-900">
                                Are you looking to raise funds for this project?
                              </span>
                              <p className="text-xs text-gray-600 mt-1">
                                Leave unchecked if you only need collaborators
                                or volunteers. Crowdfunding requires
                                institutional endorsement and M-PESA payout
                                details.
                              </p>
                            </div>
                          </label>
                            ...
                            <>
                              ...
                              <div className="space-y-1.5">
                                ...
                                <Input
                                  ...
                                  value={
                                    parsedData.basicInfo.providerMpesa || ""
                                  }
                                  onChange={(
                                    e: React.ChangeEvent<HTMLInputElement>,
                                  ) => {
                                ...
                                />
                                <p className="text-[11px] text-gray-500 italic mt-1 font-medium">
                                  Platform fee of 5% applies when funds are
                                  disbursed.
                                </p>
                              </div>
                            </>
                      ...
                      <div className="space-y-1.5 md:col-span-2">
                        ...
                        <Textarea
                          ...
                          onChange={(
                            ...
                            handleBasicInfoEdit("description", e.target.value)
                          }
                    ...
                    <div className="grid grid-cols-1 gap-4 rounded-md bg-slate-50 p-4 mt-4">
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Textarea
                            value={(
                              parsedData.eligibilityRequirements || []
                            ).join("\n")}
                            onChange={(
                              e: React.ChangeEvent<HTMLTextAreaElement>,
                            ) =>
                                ...
                                >
                                  ...
                                  <td className="border-y border-gray-100 px-5 py-5 align-top group-hover:border-blue-100">
                                    ...
                                    feat.importance === "Medium" ? (
                                      <span
                                        className={`flex items-center gap-1.5 font-bold ${feat.importance === "High" ? "text-red-600" : "text-amber-500"}`}
                                      >
                                        {feat.importance === "High" && (
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
                            ...
                            <Badge className="bg-green-100 text-green-800 hover:bg-green-200 border-0 flex items-center gap-1 py-0.5">
                              <ShieldCheck className="w-3 h-3" /> Recommended
                            </Badge>
                          </h3>
                          <p className="text-sm text-slate-500 mt-1">
                            Toggle this to collect applications directly on our
                            platform instead of an external website.
                            <br />
                            <span className="text-blue-600 font-semibold">
                              Using our internal form dramatically increases
                              your acceptance rate and builds trust with
                              applicants by preventing scams.
                            </span>
                          </p>
                        </div>
                         ...
                         <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                           ...
                           <div className="mb-4 space-y-3">
                            {customForm.fields.map((field, idx) => (
                              <div
                                key={field.id}
                                className="flex flex-wrap items-center gap-2 rounded bg-white p-3 shadow-sm border border-slate-100"
                              >
                                <Input
                                  value={field.label}
                                  onChange={(e) => {
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const newFields = customForm.fields.filter(
                                      (_, i) => i !== idx,
                                    );
                                    setCustomForm({
                                      ...customForm,
                    ...
                    {categoryShowsJobEscrow(parsedData.basicInfo.category) && (
                        ...
                        <div className="flex items-start gap-4">
                          <input
                            ...
                            onChange={(e) =>
                              setParsedData({
                                ...parsedData,
                                isEscrow: e.target.checked,
                                escrowAmount: e.target.checked
                                  ? 1000
                                  : undefined,
                              })
                            }
                          ...
                          <div>
                            <h4 className="font-semibold text-blue-900 text-base">
                              Protect this job with Escrow?
                            </h4>
                            <p className="text-sm text-blue-700">
                              We will hold the payment securely via M-PESA until
                              the work is approved. You will deposit the funds
                              via your Dashboard after submitting the job.
                            </p>
                              ...
                              <div className="mt-4 flex items-center gap-3">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setParsedData({
                                      ...parsedData,
                                      escrowAmount: Number(e.target.value),
                                    })
                                  }
                                />
                              </div>
                    ...
                    {/* Institutional Endorsement & Project Docs — required for Student & Community Projects */}
                    {categoryShowsEndorsement(
                      parsedData.basicInfo.category,
                    ) && (
                      <div className="mt-6 border border-purple-200 bg-purple-50 rounded-xl p-5 space-y-4">
                        <div className="flex flex-col gap-2">
                          <h4 className="font-semibold text-purple-900 text-lg flex items-center gap-2">
                            <ShieldCheck className="w-5 h-5" /> Project
                            Verification & Documentation
                          </h4>
                          <p className="text-sm text-purple-800">
                            Provide official documentation to verify your
                            project. Both the endorsement letter and project
                            proposal will be publicly visible to potential
                            funders and collaborators.
                          </p>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label className="text-xs font-extrabold uppercase tracking-widest text-purple-900">
                              Institution name
                            </label>
                            <Input
                              value={endorsementInstitution}
                              onChange={(e) =>
                                setEndorsementInstitution(e.target.value)
                              }
                              placeholder="e.g. KU Dept of Engineering"
                              className="bg-white"
                            />
                          </div>
                          <div className="space-y-1.5">
                            ...
                            <Input
                              value={endorsementContactTitle}
                              onChange={(e) =>
                                setEndorsementContactTitle(e.target.value)
                              }
                        ...
                        <div className="space-y-1.5 mt-4 border-t border-purple-200 pt-4">
                          <h5 className="font-semibold text-purple-900 text-sm">
                            Upload Endorsement (For Internal Verification)
                          </h5>
                          <label className="text-xs text-purple-800 font-medium">
                            Please upload the official endorsement letter/email
                            you received.
                          </label>
                          <Input
                            type="file"
                        ...
                        <div className="space-y-1.5 mt-4 border-t border-purple-200 pt-4">
                          <h5 className="font-semibold text-purple-900 text-sm">
                            Public Evidence: Project Proposal / Pitch Deck
                            (Recommended)
                          </h5>
                          <label className="text-xs text-purple-800 font-medium">
                        ...
                        <div className="space-y-1.5 mt-4 border-t border-purple-200 pt-4">
                          <h5 className="font-semibold text-purple-900 text-sm">
                            Funder Benefits / Recognitions (Optional)
                          </h5>
                          <label className="text-xs text-purple-800 font-medium">
                            What do backers get in return? (e.g. Equity for
                            commercialization, Co-authorship in research,
                            Certificates, Public Recognition)
                          </label>
                          <Textarea
                            value={funderRecognition}
                            onChange={(e) =>
                              setFunderRecognition(e.target.value)
                            }
                            placeholder="List any rewards or recognitions for those who fund or support this project..."
                    ...
                    <div className="mt-4 space-y-2 border-t border-slate-200 pt-4">
                      ...
                      <p className="text-xs text-slate-500">
                        Recommended size: 1200x630px (16:9). This will appear on
                    ...
                    {error && (
                      <p className="text-sm text-red-500 font-medium p-3 bg-red-50 border border-red-200 rounded-lg">
                        {error}
                      </p>
                    )}
                    {publishedSlug && (
                        ...
                        <div>
                          <p className="text-sm font-semibold text-green-800">
                            ...
                            {editingPostId
                              ...
                              : "Opportunity sent for verification!"}
                          </p>
                          <p className="text-xs text-green-700 mt-0.5">
                            Our admin team will review it shortly. Thank you for
                            contributing.
                          </p>
                          {editingPostId && (
                            <Button
                              ...
                              size="sm"
                              variant="link"
                              className="mt-2 text-green-700 p-0 h-auto font-bold underline"
                            >
                              <Link
                                to="/post-with-us"
            ...
            <div className="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl transform scale-100 animate-in zoom-in-95 duration-200 text-center">
              ...
              <p className="text-gray-600 mb-8 text-lg leading-relaxed">
                {editingPostId
                  ...
                  : "Opportunity submitted successfully. You will receive an email of acceptance."}
              </p>
              <button
                onClick={() => {
