    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 md:px-6 md:py-8">
        <header className="flex flex-col items-start justify-between gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => window.open('/post-with-us?admin=true', '_self')}
            >
              + Post as Admin
            </Button>
            <Button
        ...
        {loading ? (
          ...
          /* Manage Content Tab — Edit Requests + Published Posts */
          <div className="space-y-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
              <h2 className="text-xl font-bold text-slate-900">Manage Published Content</h2>
              <Button variant="outline" className="border-slate-300" onClick={() => { fetchEditRequests(); setActiveTab('opps'); }}>
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 mb-3 flex items-center gap-2">
                ...
                {editRequests.length > 0 && (
                  <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-0.5 rounded-full">{editRequests.length}</span>
                )}
              </h3>
              {editRequestsLoading ? (
                <p className="text-slate-500 text-sm">Loading edit requests...</p>
              ) : editRequests.length === 0 ? (
                <Card className="border-slate-200 shadow-sm">
                ...
                <div className="space-y-3">
                  {editRequests.map((edit: any) => (
                    <Card key={edit._id} className="border-amber-200 shadow-sm overflow-hidden">
                      <CardContent className="p-4">
                        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3">
                          <div className="space-y-1 flex-1">
                            <h4 className="font-bold text-slate-900">{edit.title}</h4>
                            <p className="text-xs text-slate-500">Poster: {edit.posterEmail} · {new Date(edit.submittedAt).toLocaleDateString()}</p>
                            {edit.changeReason && (
                              <p className="text-sm text-amber-700 bg-amber-50 rounded p-2 mt-1">
                                <span className="font-semibold">Reason:</span> {edit.changeReason}
                              </p>
                            )}
                            {/* Diff view */}
                            {Object.keys(edit.diff).length > 0 && (
                              <div className="mt-2 bg-slate-50 rounded p-2 space-y-1">
                                <p className="text-[10px] font-bold uppercase text-slate-500">Changes:</p>
                                {Object.entries(edit.diff).map(([field, values]: [string, any]) => (
                                  <div key={field} className="text-xs">
                                    <span className="font-semibold text-slate-600">{field}:</span>
                                    <span className="text-red-500 line-through ml-1">{values.old || '(empty)'}</span>
                                    <span className="text-green-600 ml-1">→ {values.new || '(empty)'}</span>
                                  </div>
                                ))}
                              </div>
                            )}
                          ...
                          <div className="flex gap-2 shrink-0">
                            <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white" onClick={() => handleApproveEditRequest(edit._id)} disabled={actionLoading === `approve_edit_${edit._id}`}>
                              <CheckCircle className="w-3 h-3 mr-1" /> Approve
                            </Button>
                            <Button size="sm" variant="outline" className="border-red-200 text-red-600 hover:bg-red-50" onClick={() => handleRejectEditRequest(edit._id)} disabled={actionLoading === `reject_edit_${edit._id}`}>
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 mb-3 flex items-center gap-2">
                ...
                Published Posts ({allOpps.length})
              </h3>
              {allOpps.length === 0 ? (
                <Card className="border-slate-200 shadow-sm">
                  <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <Card key={opp.id} className="border-slate-200 shadow-sm overflow-hidden flex flex-col">
                    <div className="flex flex-col sm:flex-row items-center p-4 gap-4">
                      ...
                      <div className="flex gap-2 shrink-0 flex-wrap justify-end mt-2 sm:mt-0">
                        <Button variant="outline" size="sm" onClick={() => handleViewOppDetail(opp.id)} disabled={oppDetailLoading === opp.id}>
                          {oppDetailLoading === opp.id ? 'Loading...' : expandedOppDetail === opp.id ? 'Hide' : 'Details'}
                        </Button>
                    ...
                    {expandedOppDetail === opp.id && oppDetailData && (
                      <div className="border-t border-slate-100 bg-slate-50/50 p-4 sm:p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          <div className="bg-blue-50 border border-blue-100 p-3 rounded-lg text-center">
                            ...
                            <span className="text-lg font-bold text-slate-900">{oppDetailData.views || 0}</span>
                          ...
                          <div className="bg-emerald-50 border border-emerald-100 p-3 rounded-lg text-center">
                            ...
                            <span className="text-lg font-bold text-slate-900">{oppDetailData.clicks || 0}</span>
                          ...
                          <div className="bg-purple-50 border border-purple-100 p-3 rounded-lg text-center">
                            ...
                            <span className="text-lg font-bold text-slate-900">{oppDetailData.ctr || 0}%</span>
                          ...
                          <div className="bg-amber-50 border border-amber-100 p-3 rounded-lg text-center">
                            ...
                            <span className="text-lg font-bold text-slate-900">{oppDetailData.applicantGroups ? Object.values(oppDetailData.applicantGroups).flat().length : 0}</span>
                            <p className="text-[10px] text-slate-500 uppercase font-semibold">Applicants</p>
                          </div>
                        </div>

                        {/* Cover Image */}
                        {oppDetailData.logoUrl && (
                          <div className="flex justify-center">
                            <div className="w-32 h-32 bg-white border border-slate-200 rounded-lg overflow-hidden">
                              <img
                                src={oppDetailData.logoUrl.startsWith('/images/') ? (API_BASE.replace('/api', '') + oppDetailData.logoUrl) : oppDetailData.logoUrl}
                                alt="Cover"
                                className="w-full h-full object-contain p-2"
                                onError={(e: any) => { e.target.src = "/Opportunities Kenya Logo 2.png"; }}
                              />
                        ...
                        {oppDetailData.description && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-1">Description</h4>
                            <p className="text-sm text-slate-700">{oppDetailData.description}</p>
                          </div>
                        )}

                        {oppDetailData.fullDescription && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-1">Full Description</h4>
                            <div className="text-sm text-slate-700 whitespace-pre-wrap bg-white p-3 rounded border border-slate-100">{oppDetailData.fullDescription}</div>
                          </div>
                        )}

                        {oppDetailData.benefits?.length > 0 && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-1">Benefits</h4>
                            <ul className="list-disc pl-5 text-sm text-slate-700 space-y-0.5">
                              {oppDetailData.benefits.map((b: string, i: number) => <li key={i}>{b}</li>)}
                            </ul>
                          </div>
                        )}

                        {oppDetailData.eligibility?.requirements?.length > 0 && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-1">Eligibility</h4>
                            <ul className="list-disc pl-5 text-sm text-slate-700 space-y-0.5">
                              {oppDetailData.eligibility.requirements.map((r: string, i: number) => <li key={i}>{r}</li>)}
                            </ul>
                          </div>
                        )}

                        {/* Applicant Accordion */}
                        {oppDetailData.applicantGroups && (
                          <div>
                            <h4 className="text-xs font-bold text-slate-500 uppercase mb-2">Applicants by Status</h4>
                            <div className="space-y-1">
                              {Object.entries(oppDetailData.applicantGroups).map(([status, emails]: [string, any]) => (
                                <details key={status} className="group bg-white border border-slate-100 rounded">
                                  <summary className="px-3 py-2 cursor-pointer text-sm font-medium text-slate-700 flex items-center justify-between hover:bg-slate-50">
                                    <span className="capitalize">{status}</span>
                                    <span className="text-xs bg-slate-100 px-2 py-0.5 rounded-full">{emails.length}</span>
                                  </summary>
                                  <div className="px-3 pb-2 pt-1 border-t border-slate-50">
                                    {emails.length > 0 ? (
                                      <div className="flex flex-wrap gap-1 mt-1">
                                        {emails.map((email: string, i: number) => (
                                          <span key={i} className="text-[10px] bg-slate-100 px-2 py-0.5 rounded text-slate-600 font-mono">{email}</span>
                                        ))}
                                      </div>
                                    ) : (
